...
class PolicyGenerator {
    ...
    generateFromMCPConfig(config, policy) {
        let riskReduction = 0;
        const changes = [];
        ...
        for (const [serverName, serverConfig] of Object.entries(config.mcpServers)) {
            ...
            if (serverConfig.args && Array.isArray(serverConfig.args)) {
                const fsPath = serverConfig.args.find((arg) => typeof arg === 'string' && (arg === '/' || arg.includes('*')));
                if (fsPath === '/') {
    ...
    generateFromAgentTools(tools, policy) {
        let riskReduction = 0;
        const changes = [];
    ...
    isDangerousTool(name) {
        ...
        return dangerousNames.some(dangerous => name.toLowerCase().includes(dangerous));
    }
