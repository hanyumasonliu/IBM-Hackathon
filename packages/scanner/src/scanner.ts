...
export class MCPShieldScanner {
  ...
  /**
   * Scan an MCP config, agent tools, or prompt text
   */
  async scan(input: ScanInput): Promise<ScanResult> {
    ...
    // Generate suggested policy if there are findings
    let suggested_policy: SafePolicy | undefined;
  ...
  /**
   * Detect all security issues in the input
   */
  private detectIssues(input: ScanInput): Finding[] {
    ...
    if (typeof input === 'string') {
      ...
      try {
        ...
        // If it's not JSON, treat it as prompt text
        return this.scanPromptText(originalString);
      }
    }

    // Check if it's an agent tools config
    if (this.isAgentToolsConfig(input)) {
      return scanAgentTools(input.tools);
    }

    // Otherwise treat as MCP config
    return scanMCPConfig(input as MCPConfig);
  }

  /**
   * Check if input is agent tools config
   */
  private isAgentToolsConfig(input: any): input is AgentToolsConfig {
    return input && Array.isArray(input.tools);
  }

  /**
   * Scan prompt text for injection patterns
   */
  private scanPromptText(text: string): Finding[] {
   ...
   */
  private countSeverities(findings: Finding[]): SeverityCounts {
    return {
   ...
   */
  private calculateRiskScore(counts: SeverityCounts): number {
    let score = 0;
  ...
  /**
   * Generate a safe policy from findings
   */
  private generatePolicy(input: ScanInput, findings: Finding[]): SafePolicy {
    ...
    if (typeof input === 'string') {
      try {
        ...
        return policy; // Can't generate policy from unparseable text
      }
    }

    // Analyze findings to generate policy
    ...
    // Set approval requirement based on severity
    ...
    // Generate max scope explanation
